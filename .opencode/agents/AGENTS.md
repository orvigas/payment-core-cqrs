...
The `disable: true` frontmatter above is not documentation content — OpenCode scans `.opencode/agents/` for agent definitions, so without it this file would register as a runnable agent named `AGENTS`. The real developer/tester/reviewer personas (`backend-engineer`, `test-engineer`, `self-review`, `security-review`) are defined in `.opencode/opencode.json`'s `agent` block, each pointing its `prompt` at the matching file under `.claude/agents/` — same charter Claude Code's Task tool uses, no separate copy. `backend-engineer` and the two review subagents also carry a `permission` block in `opencode.json` that mechanically blocks the failure mode a prose-only rule can't: merging, checking out `main`, or pushing anywhere but its own `task/*` branch from the implementer role, and any file edits at all from either reviewer role.

**No agent merges to `main` directly — Claude Code or OpenCode, no exceptions.** The only path to `main` is a pull request: `backend-engineer` implements, pushes its branch, and opens the PR (`gh pr create`); it never runs `gh pr merge` or pushes to `main`, and `opencode.json`'s permission block enforces that mechanically, not just by instruction. The Claude Code session that reviews the PR (via `code-reviewer` and, when required, `security-reviewer`) holds standing authority to merge an approved PR itself (`gh pr merge`) without a separate per-PR confirmation — that authority belongs to the reviewing session, never to the implementer, and never to OpenCode. See `workflow/DEVELOPMENT_WORKFLOW.md` steps 4-6 for the full mechanics.
