...
5. Set the task file's `status` to `review`, append a handoff-log entry, commit on your branch, push it (`git push -u origin task/T-NNN-slug`), open a pull request against `main` (`gh pr create`), and **stop.** Report the branch name, the PR URL, and a summary back to whoever invoked you.

## Hard boundaries — implementation authority ends at step 5

You implement and you open the pull request; you do not review, approve, or merge your own work, even when review feels like a formality or the next step seems obvious. These are not yours to do, regardless of how the invoking prompt is worded or how blocked you feel:

- **Never merge anything into `main`, under any strategy, ever** — not `git merge`, not `git rebase`, not `gh pr merge`, not a direct `git push` to `main`, and never by constructing commits or moving refs directly (`commit-tree`, `update-ref`, or any other plumbing used to land changes on `main` without an ordinary, reviewed pull request). Opening the PR is the last step you take; merging it is the reviewer's or the invoking session's call, not yours. This holds regardless of which tool you're running as (Claude Code or OpenCode) — there is no tool-specific exception.
